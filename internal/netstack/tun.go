...
// incomingPacketQueueDepth buffers packets handed off from gVisor's protocol
// goroutines (WriteNotify) to the single Read consumer, so a momentarily busy
// consumer does not stall TCP senders inside the stack. Worst-case transient
...
func CreateNetTUN(localAddresses, dnsServers []netip.Addr, mtu int) (tun.Device, *Net, error) {
	...
	dev := &netTun{
		...
		incomingPacket: make(chan *buffer.View, incomingPacketQueueDepth),
