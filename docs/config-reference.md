...
| `socks.l4_max_conn_failures` | L4 only. How many CONNECT handshakes may fail **in a row with no Cloudflare answer** before the shared QUIC connection is declared *wedged* (alive at the QUIC layer — keepalive still flows so `OpenRequestStream` keeps succeeding — but no longer answering CONNECTs) and rebuilt. Default `50`. Lower it for a faster forced rebuild while experimenting; too low risks re-tripping on a transient blip. A single Cloudflare answer (a `2xx` or a per-target rejection) resets the run, so an occasional slow/blackholed target among healthy traffic never trips it. This count catches a wedge quickly under high concurrency; a complementary **elapsed-time trip** (a no-answer run older than `2 × l4_connection_timeout`) bounds recovery under low concurrency, where reaching the count alone could otherwise take many serial connect-timeout windows. `<=0` uses the default. |
