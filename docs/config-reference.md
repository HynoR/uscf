...
| `socks.l4` | Use L4 mode: tunnel each TCP flow as an HTTP/3 CONNECT stream over a single shared QUIC connection, bypassing the userspace netstack. Faster and lighter, but **TCP-only** (Cloudflare's MASQUE proxy endpoint refuses `connect-udp` with HTTP 403) and DNS is always resolved locally (`remote_dns` ignored). Mutually exclusive with `http2`. CLI override: `uscf proxy --l4`. |
| `socks.l4_pool_size` | L4 only. How many shared QUIC connections to keep. **Default `1`** — a single connection is one stable WARP egress identity. A larger pool is **not** a throughput knob: Cloudflare caps concurrent QUIC connections per enrollment (extras are reaped down to ~1-3), and every connection is a *different* egress IP, so a pool makes flows hop egress IPs for no real headroom. Per-connection `MAX_STREAMS` is handled by `l4_max_streams` instead. Raise only if you understand and accept egress-IP churn. `<=0` uses the default; capped at `64`. |
| `socks.l4_max_streams` | L4 only. Hard ceiling on concurrent in-flight CONNECT streams. When reached, a new dial **fails fast** (the client retries) instead of blocking on the shared connection's `MAX_STREAMS` — which is what otherwise lets inbound SOCKS connections pile up into an OOM. It is an OOM backstop sized well above typical peak concurrency, not the normal operating limit. A persistent stream of saturation warnings (`l4 saturated: rejecting new flows fast`) means one enrollment cannot carry the offered concurrency — consider L3 mode. Default `4096`; `<=0` uses the default; capped at `65536`. The live stream count and cumulative rejections are logged every 30s (at `debug`, or `info` on any interval where saturation occurred). |
| `socks.l4_half_open_timeout` | L4 only. How long a half-open TCP relay's surviving direction may stay **idle** before it is reclaimed, once the other direction has finished. It re-arms on activity, so an active/slow-but-streaming response is never cut — only a truly idle half-open flow is reaped. This keeps a finished/abandoned flow from pinning its QUIC stream (against `MAX_STREAMS`) for the full `idle_timeout`, which under load would otherwise exhaust the shared connection and hang the proxy. Default `30s`; `<=0` uses the default. |
