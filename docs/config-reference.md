...
| `socks.l4` | Use L4 mode: tunnel each TCP flow as an HTTP/3 CONNECT stream over a small pool of shared QUIC connections, bypassing the userspace netstack. Faster and lighter, but **TCP-only** (Cloudflare's MASQUE proxy endpoint refuses `connect-udp` with HTTP 403) and DNS is always resolved locally (`remote_dns` ignored). Mutually exclusive with `http2`. CLI override: `uscf proxy --l4`. |
| `socks.l4_pool_size` | L4 only. How many shared QUIC connections to spread streams across. Each TCP flow is one QUIC stream, and a single QUIC connection has a peer-imposed concurrent-stream ceiling (`MAX_STREAMS`); a pool multiplies that ceiling so high concurrency (thousands of simultaneous flows) does not saturate one connection and stall new dials. Costs roughly one set of QUIC receive windows per connection. Default `8`; `<=0` uses the default; capped at `64`. |
